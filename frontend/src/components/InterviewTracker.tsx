...
interface InterviewTrackerProps {
  ...
  onUpdateStatus: (id: string, status: 'Scheduled' | 'Completed' | 'Cancelled' | 'Passed' | 'Failed') => Promise<void>;
}

export default function InterviewTracker({ interviews, jobs, onAddInterview, onUpdateStatus }: InterviewTrackerProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Manage scheduled interview rounds, structure system design notes, and trace progress results.</p>
        </div>
      ...
      {showAddForm && (
        <form onSubmit={handleSubmit} className="bg-slate-50 p-5 rounded-xl border border-slate-200 shadow-inner space-y-4 max-w-2xl">
          <h3 className="text-xs font-bold tracking-wider uppercase text-slate-500">Record Upcoming Interview Round</h3>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-[11px] font-bold text-slate-600 uppercase">Linked Company / Job</label>
              <select
                value={selectedJobId}
            ...
            <div className="space-y-1">
              ...
              >
                ...
                <option value="Technical Take-home Assessment">Technical Take-home Assessment</option>
          ...
          <div className="space-y-1">
            <label className="text-[11px] font-bold text-slate-600 uppercase">Preparation / Study Notes</label>
            <textarea
              value={notes}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {interviews.length === 0 ? (
          <div className="col-span-full bg-slate-50 p-12 text-center rounded-xl border border-dashed border-slate-200 space-y-3">
            ...
            <p className="text-xs text-slate-400 max-w-sm mx-auto">Click "Schedule Interview" above to log and prepare for your upcoming corporate hiring rounds.</p>
          </div>
        ) : (
          interviews.map(i => (
            <div key={i.id} className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden flex flex-col justify-between hover:border-indigo-100 hover:shadow-md transition-all">
              {/* Header card state */}
              <div className="p-5 border-b border-slate-50">
                <div className="flex justify-between items-start gap-2">
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold uppercase ${
                    i.status === 'Scheduled' ? 'bg-sky-50 text-sky-700 border border-sky-100' :
                    i.status === 'Passed' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' :
                    i.status === 'Failed' ? 'bg-rose-50 text-rose-700 border border-rose-100' :
                    'bg-slate-100 text-slate-600'
                  }`}>
                    {i.status}
                  </span>
                  
                  <span className="text-[10px] font-mono text-slate-400 font-semibold uppercase">{i.type}</span>
                </div>
                ...
                <div className="flex items-center gap-2 mt-4 text-[11px] font-mono text-slate-500 bg-slate-50 p-2 rounded-lg border border-slate-100">
                  ...
                  <span>{new Date(i.date).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}</span>
                </div>
              </div>
              ...
              {i.notes && (
                <div className="p-5 bg-slate-50/50 flex-1 border-b border-slate-50">
                  ...
                  <p className="text-xs text-slate-600 line-clamp-4 whitespace-pre-wrap leading-relaxed">{i.notes}</p>
                </div>
              )}
