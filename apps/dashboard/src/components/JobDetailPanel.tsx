...
function ChipList({
  ...
  const styles =
    tone === 'good'
      ? 'bg-emerald-50 border-emerald-100 text-emerald-700 dark:bg-emerald-950/50 dark:border-emerald-900 dark:text-emerald-300'
      : tone === 'warn'
        ? 'bg-amber-50 border-amber-100 text-amber-700 dark:bg-amber-950/50 dark:border-amber-900 dark:text-amber-300'
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 p-6 shadow-sm sticky top-6 space-y-6 max-h-[calc(100vh-8rem)] overflow-auto">
      <div className="flex justify-between items-start gap-3 border-b border-slate-50 dark:border-slate-800 pb-4">
        ...
        <button
          onClick={onClose}
          className="text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-400 p-1 rounded-lg"
        >
          <X className="h-4 w-4" />
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 text-[11px]">
        <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-3">
          <div className="text-slate-400 dark:text-slate-500 uppercase font-bold text-[10px]">
            Stack
          </div>
        ...
        <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-3">
          <div className="text-slate-400 dark:text-slate-500 uppercase font-bold text-[10px] flex items-center gap-1">
      ...
      {onUpdateStatus && (
        <form
          ...
          className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4 space-y-3"
        >
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] px-2 py-0.5 rounded font-bold uppercase bg-indigo-50 dark:bg-indigo-950/50 text-indigo-700 dark:text-indigo-300">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <select
              ...
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-2.5 py-1.5 text-xs text-slate-700 dark:text-slate-300 font-semibold focus:outline-none focus:ring-1 focus:ring-indigo-500"
            >
          ...
          <input
            ...
            className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5 text-xs text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-1 focus:ring-indigo-500"
          />
        </form>
      )}
        ...
        <div className="space-y-3 border-t border-slate-100 dark:border-slate-800 pt-4">
          ...
          <div className="space-y-2 max-h-48 overflow-auto pr-1">
            {job.actionHistory.map((entry, idx) => (
              <div
                key={idx}
                className="bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-lg p-2.5 text-xs space-y-1"
              >
                ...
                {entry.note && (
                  <p className="text-xs text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-2 rounded italic">
                    "{entry.note}"
                  </p>
                )}
              </div>
      ...
      <div className="bg-gradient-to-br from-indigo-50 to-sky-50/50 dark:from-indigo-950/50 dark:to-sky-950/30 rounded-xl border border-indigo-100/30 dark:border-indigo-900/50 p-4.5 space-y-4">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-1.5 text-xs font-bold text-indigo-800 dark:text-indigo-300 uppercase tracking-wide">
            <Sparkles className="h-4 w-4 text-indigo-500" />
            <span>Match Analysis</span>
          </div>
          {job.matchScorer && (
            <span className="text-[10px] uppercase font-bold text-indigo-600 dark:text-indigo-400 bg-white/70 dark:bg-slate-900/70 px-2 py-0.5 rounded">
              {job.matchScorer}
            </span>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          <ul className="space-y-2">
            {insights.resumeSuggestions.map((item) => (
              <li
                key={item}
                className="text-xs text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-lg p-2.5"
              >
                {item}
              </li>
            ))}
      ...
      <div className="space-y-2.5">
        {(job.applicationUrl || job.url) && (
          <a
            ...
            className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs py-2.5 rounded-lg font-semibold uppercase flex items-center justify-center gap-1 hover:bg-slate-50 dark:hover:bg-slate-800"
          >
            Apply / View Posting
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-2.5 text-xs min-h-[80px]"
        />
      </div>
      ...
      <div className="space-y-2 border-t border-slate-50 dark:border-slate-800 pt-4">
        ...
        <div className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed max-h-[240px] overflow-auto whitespace-pre-wrap">
          {job.description}
        </div>
      </div>
    </div>
