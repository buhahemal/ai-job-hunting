...
| Module | Role | Hosting |
|--------|------|---------|
| `frontend/` → `apps/dashboard/` | React dashboard | GitHub Pages |
| `backend/` → `apps/api/` | API + profile (interim JSON) | Local dev / Supabase |
| `scraper/` → `scanners/` + `packages/scanner-sdk/` | Job discovery pipeline | GitHub Actions cron |
...
| 2 | Repository + CI/CD + standards (in progress) |
| 3 | Supabase schema |
...
Current paths (`frontend/`, `backend/`, `scraper/`) remain supported during migration.
